...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 w-full ${
      scrolled ? "py-4 glass-light dark:glass-dark shadow-md" : "py-6 bg-transparent"
    }`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl border border-slate-200 hover:bg-slate-100 dark:border-slate-800 dark:hover:bg-slate-900 text-slate-500 dark:text-slate-400 cursor-pointer"
          ...
          <Button variant="primary" size="sm" rightIcon={<ArrowRight size={16} />} onClick={() => navigate("/register")}>
            Get Started
          </Button>
        </div>
        ...
        <div className="flex md:hidden items-center gap-3">
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="p-2 text-slate-700 dark:text-slate-300 cursor-pointer">
          ...
          >
            <div className="px-6 py-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-3 pt-2">
                <Button variant="outline" size="md" onClick={() => { setIsOpen(false); navigate("/login"); }} className="w-full">
                  Sign In
                </Button>
                <Button variant="primary" size="md" rightIcon={<ArrowRight size={18} />} onClick={() => { setIsOpen(false); navigate("/register"); }} className="w-full">
                  Get Started
                </Button>
              </div>
            </div>
