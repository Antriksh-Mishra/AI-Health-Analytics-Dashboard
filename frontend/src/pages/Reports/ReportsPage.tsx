...
export default function ReportsPage() {
  ...
  const handleEmailDoctor = () => {
    ...
    let reportSummary = `Dear Doctor,\n\nI am sharing my clinical biometric report summary generated by MedIntel AI:\n`;
    reportSummary += `Report Date: ${new Date(selectedReport.uploaded_at).toLocaleDateString()}\n\n`;
    reportSummary += `Extracted Lab Values:\n`;
    
    if (bio) {
      reportSummary += `- Fasting Glucose: ${bio.blood_sugar !== null ? `${bio.blood_sugar} mg/dL` : "Not detected"}\n`;
      reportSummary += `- Hemoglobin: ${bio.hemoglobin !== null ? `${bio.hemoglobin} g/dL` : "Not detected"}\n`;
      reportSummary += `- Cholesterol: ${bio.cholesterol !== null ? `${bio.cholesterol} mg/dL` : "Not detected"}\n`;
      reportSummary += `- Vitamin D: ${bio.vitamin_d !== null ? `${bio.vitamin_d} ng/mL` : "Not detected"}\n`;
      reportSummary += `- Blood Pressure: ${bio.systolic_bp !== null && bio.diastolic_bp !== null ? `${bio.systolic_bp}/${bio.diastolic_bp} mmHg` : "Not detected"}\n\n`;
    }
    
    if (selectedReport.ai_insights) {
      reportSummary += `AI Analysis Clinical Summary:\n${selectedReport.ai_insights.summary}\n\n`;
      reportSummary += `AI Alert Flags:\n`;
      selectedReport.ai_insights.flagged_items.forEach(flag => {
        reportSummary += `- [${flag.metric}]: ${flag.value} (Ref: ${flag.range}) - ${flag.reason}\n`;
      });
      reportSummary += `\nWellness Recommendations:\n`;
      selectedReport.ai_insights.wellness_recommendations.forEach(rec => {
        reportSummary += `- ${rec}\n`;
      });
    }
    
    reportSummary += `\nDisclaimer: MedIntel AI is a clinical supportive tracking tool. Final medical determinations and prescription adjustments must be reviewed by a licensed provider.\n\nBest regards,\n${username}`;

    const subject = encodeURIComponent(`MedIntel Health Summary - ${username}`);
      ...
      {detailModalOpen && selectedReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh]">
            <header className="px-8 py-5 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-left flex-wrap gap-4 print-hide-header">
              <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-150 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition-all border border-slate-250/20 shadow-sm cursor-pointer"
                >
                  <Printer size={12} />
                  Print PDF
                </button>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/20 dark:hover:bg-blue-950/40 text-blue-600 dark:text-blue-400 transition-all border border-blue-200/20 shadow-sm cursor-pointer"
                >
                  <Mail size={12} />
                  Email Doctor
                </button>
                <button 
                  ...
                  className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400 hover:text-slate-600 transition-colors cursor-pointer ml-2"
                >
                  ✕
                </button>
              </div>
